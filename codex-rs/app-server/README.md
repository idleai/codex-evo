...
To continue through a different provider profile without sending source-provider tool or reasoning
items to the target, request a portable handoff fork:

```json
{ "method": "thread/fork", "id": 13, "params": { "threadId": "thr_123", "profile": "dsv4", "portableHistory": true } }
```

The fork remains linked through `forkedFromId` and emits the normal `thread/started` notification,
so existing app-server and remote-control subscribers discover it through the standard thread
lifecycle.
