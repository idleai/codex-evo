...
- Experimental `thread/fork` provider handoffs use `profile` to select `$CODEX_HOME/<profile>.config.toml` on the app-server host (`null` selects the base config) and `portableHistory: true` to copy a bounded user/assistant text projection while dropping provider-specific reasoning and tool items.
