...
impl App {
    pub(crate) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::HandoffCurrentSession { profile } => {
                self.session_telemetry.counter(
                    "codex.thread.fork",
                    /*inc*/ 1,
                ...
                if let Some(thread_id) = self.chat_widget.thread_id() {
                    match app_server
                        .handoff_thread(&self.local_settings, self.config.clone(), thread_id, profile)
                        .await
                    {
                        Ok(handoff) => {
                            ...
                            let model_picker_catalog_available = self
                                ...
                                .try_list_models()
                                .is_ok_and(|models| {
                                    models.iter().any(|preset| preset.model == model)
                                });
                            ...
                            {
                                Ok(()) => {
                                    self.chat_widget.set_model_picker_catalog_available(
                                        model_picker_catalog_available,
                                    );
                                    self.chat_widget.add_info_message(
                                        format!(
                                    ...
                                    if let Some(summary) = summary {
                                        ...
                                        if let Some(command) = summary.resume_hint {
                                            lines.push(
                                                vec![
                                                    "To continue the previous provider thread, run "
                                                        .into(),
                                                    command.cyan(),
                                                ]
                                                .into(),
                    ...
                    self.chat_widget.add_error_message(
                        "The session is still starting; try /handoff again in a moment."
                            .to_string(),
                    );
                }
